...
// ----------------------------------------------------------------
VS_OUTPUT vs_customMain ( VS_INPUT Input)
{
	VS_OUTPUT Output;
...
float4 ps_gris(float2 Texcoord: TEXCOORD0, float4 Color:COLOR0) : COLOR0
{
	...
	//Variaciones de gris
	retColor.x = retColor.x *0.222 + 10*cos(time);
	retColor.y = retColor.y *0.707 + 10*cos(time);
	retColor.z = retColor.z *0.071 + 10*cos(time);
...
technique ShaderTerror
{
	pass Pass_0
	{
		VertexShader = compile vs_2_0 vs_customMain();
